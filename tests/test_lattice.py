...
class LatticeTests(unittest.TestCase):
    def test_init_creates_specs_skill_and_agents_section(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertTrue(
                (root / "lattice/specs/rules/PROJECT-RULE-001.json").exists()
            )
            self.assertTrue(
    ...
    def test_unit_page_renders_fields_before_collapsed_raw_json(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            html = (root / "lattice/generated/docs/units/todo-item-001.html").read_text(
                encoding="utf-8"
            )
            lifecycle_html = (
            ...
            self.assertIn(
                '<lattice-link type="data_type" lattice-id="TODO-DATA-TYPE-001" variant="field-type">string</lattice-link>',
                html,
            )
            self.assertIn(
                '<lattice-link type="lifecycle_type" lattice-id="TODO-LIFECYCLE-001" variant="field-type">TodoStatus</lattice-link>',
                html,
            )
            self.assertIn("Invariants", html)
            self.assertIn("TodoItem belongs to one list", html)
            ...
            self.assertIn(
                '<lattice-link type="lifecycle_value" lattice-id="TODO-STATUS-001" variant="plain">Open</lattice-link>',
                lifecycle_html,
            )
            self.assertIn("Open means work has not started yet.", lifecycle_html)
            self.assertNotIn("Related Concepts", html)
            self.assertIn(
                '<lattice-link type="concept" lattice-id="TODO-CONCEPT-001" variant="plain">',
                html,
            )
            visible_links = html.split('<lattice-links-panel slot="links">', 1)[
                1
            ].split("</lattice-links-panel>", 1)[0]
            self.assertIn("TodoItem belongs to one list", visible_links)
