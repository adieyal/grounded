...
This document explains the intent behind Grounded. The canonical project-memory
facts live in `grounded/specs`; this overview is grounded in
`GROUNDED-THEORY-001`, `PROJECT-CONCEPT-001`, and `PROJECT-RULE-001`.

## What Grounded Is

Grounded is executable project memory.
...
drift. Grounded treats that drift as something to detect and repair.

Grounded is not a runtime framework, business rule engine, or replacement for
implementation. Code remains the thing that runs. Tests remain the proof that
behavior works. Grounded provides the declared intent and coordination surface
around them.
...
That graph is what lets Grounded validate not only document shape, but also
...
Grounded exists to reduce that coordination cost. It gives humans and LLMs a
compact, auditable source of project truth, then lets tooling validate references,
coverage, generated context, and drift.

## Core Model

Grounded separates four concerns that are often blurred together:
...
repo can define its own domain terms on top of Grounded, but the kernel should
...
This boundary matters. A Grounded spec can say that a business rule exists, name
...
Grounded should focus on durable semantics whose drift is expensive:
...
Grounded should stay boring in implementation and ambitious in effect.
...
logic. Grounded should distinguish proven facts, partial checks, advisory findings,
and human judgment.

Wherever a semantic claim can be checked deterministically, Grounded should check
...
Grounded is especially useful in LLM-era development because LLMs need durable,
...
Grounded should avoid becoming:
...
Those boundaries are important. If Grounded tries to execute the project, manage
...
Grounded is a small system for keeping project meaning stable.
