...
def tag_output_path(config: GroundedConfig, tag: str) -> Path:
...
def tags_by_name(
    specs: list[Spec], config: GroundedConfig, from_path: Path
...
def tag_sections_for(
    tag: str, specs: list[Spec], config: GroundedConfig, from_path: Path
) -> list[dict[str, Any]]:
