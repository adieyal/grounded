...
<head>
  ...
  <script>
    (() => {
      try {
        const theme = window.localStorage.getItem("grounded-theme");
...
<body>
  <script type="application/json" id="grounded-registry">{{ grounded_registry_json | safe }}</script>
  <script type="application/json" id="grounded-search-index">{{ search_index_json | safe }}</script>
  <script type="application/json" id="grounded-tag-index">{{ tag_index_json | safe }}</script>
  <grounded-docs-app>
    <grounded-top-bar slot="top" home-href="{{ docs_home_href }}" label="{{ docs_nav_label }}">
    ...
    <grounded-sidebar slot="nav" aria-label="Knowledge units">
      <grounded-nav-group>
        ...
        <grounded-nav-item tone="flow"><a class="plain-nav-link" href="{{ document_graph_href }}">Document Graph</a></grounded-nav-item>
        <grounded-nav-item tone="flow"><a class="plain-nav-link" href="{{ artifact_index_href }}">Artifacts</a></grounded-nav-item>
        <grounded-nav-item tone="meta"><a class="plain-nav-link" href="{{ background_href }}">Background</a></grounded-nav-item>
      </grounded-nav-group>
      {% for type_name, units in by_type.items() %}
      <grounded-nav-group{% if current_spec and current_spec.kind == type_name %} open{% endif %}>
        <span slot="label">{{ type_nav_label(type_name) }}</span>
        {% for nav_spec in units %}
        <grounded-nav-item tone="{{ type_tone(nav_spec.kind) }}"{% if current_spec and current_spec.id == nav_spec.id %} active{% endif %}>
          {{ grounded_link(nav_spec.kind, nav_spec.id, display_name(nav_spec), "nav") | safe }}
        </grounded-nav-item>
        {% endfor %}
      </grounded-nav-group>
      {% endfor %}
    </grounded-sidebar>
    <grounded-main slot="main" class="pd-body">
...
<section class="story-band">
  ...
  {% for doc in docs %}
    <article class="artifact-card">
      ...
      <h3>{{ grounded_link(doc.kind, doc.id, display_name(doc), "plain") | safe }}</h3>
...
{% if story_specs %}
<grounded-unit-section aria-labelledby="story-specs-title">
  <grounded-section-heading divider id="story-specs-title">What The Graph Says</grounded-section-heading>
...
</grounded-unit-section>
{% endif %}
{% for type_name, units in by_type.items() %}
<grounded-unit-section aria-labelledby="{{ type_name }}-title">
  <grounded-section-heading divider id="{{ type_name }}-title">{{ type_nav_label(type_name) }}</grounded-section-heading>
  <div class="pd-cards">
  {% for spec in units %}
    <grounded-unit-card>
      <h3 class="pd-card-name nm-{{ type_tone(spec.kind) }}">{{ grounded_link(spec.kind, spec.id, display_name(spec), "card-title") | safe }}</h3>
...
<section class="story-band">
  <div>
    ...
    <p>This view keeps Grounded core definitions, guardrails, and support specs available without forcing them to compete with the primary project narrative.</p>
  </div>
...
{% for type_name, units in by_type.items() %}
<grounded-unit-section aria-labelledby="{{ type_name }}-background-title">
  <grounded-section-heading divider id="{{ type_name }}-background-title">{{ type_nav_label(type_name) }}</grounded-section-heading>
  <div class="pd-cards">
  {% for spec in units %}
    <grounded-unit-card>
      <h3 class="pd-card-name nm-{{ type_tone(spec.kind) }}">{{ grounded_link(spec.kind, spec.id, display_name(spec), "card-title") | safe }}</h3>
...
{% block title %}{{ data.name }} · Grounded{% endblock %}
{% block content %}
...
{% if tags %}
<div class="tag-panel">
  <grounded-section-heading>Tags</grounded-section-heading>
  <div class="tag-list">
...
<div slot="fields">
    ...
    <grounded-section-heading>Fields</grounded-section-heading>
    <grounded-field-table>
    ...
    {% for field in rows %}
      <tr class="field-row" id="{{ field_anchor(spec.id, field['name']) }}">
        ...
        <td class="ft-desc">
          ...
          <div class="allowed-values">
            <span>Tags:</span>
            {% for tag in field["tags"] %}{{ grounded_link("tag", tag, tag, "tag") | safe }}{% endfor %}
          </div>
          {% endif %}
          {% if field["references"] %}
          <div class="allowed-values">
            ...
            {% for reference_id in field["references"] %}
              {% if reference_id in registry.by_id %}
                ...
                {{ grounded_link(reference.kind, reference.id, display_name(reference), "plain") | safe }}
              {% else %}
    ...
    <grounded-concept-section slot="context" aria-labelledby="{{ section["role"] or "related" }}-concepts-title">
      <grounded-section-heading id="{{ section["role"] or "related" }}-concepts-title">{{ section["title"] }}</grounded-section-heading>
      {% for node in section["items"] %}
      <grounded-concept-card>
        <h3 class="pd-inv-name">{{ grounded_link(node.type, node.id, node.label, "plain") | safe }}</h3>
        <p class="pd-inv-desc">{{ rich_text(node.summary, registry) | safe }}</p>
      </grounded-concept-card>
      {% endfor %}
    </grounded-concept-section>
    {% endfor %}
    <grounded-links-panel slot="links">
      <grounded-section-heading>Links</grounded-section-heading>
...
{% for type_name, units in by_type.items() %}
<grounded-unit-section aria-labelledby="{{ type_name }}-slice-title">
  <grounded-section-heading divider id="{{ type_name }}-slice-title">{{ type_nav_label(type_name) }}</grounded-section-heading>
  <div class="pd-cards">
  {% for spec in units %}
    <grounded-unit-card>
      <h3 class="pd-card-name nm-{{ type_tone(spec.kind) }}">{{ grounded_link(spec.kind, spec.id, display_name(spec), "card-title") | safe }}</h3>
...
<grounded-unit-section>
  <grounded-section-heading divider>{{ section["title"] }}</grounded-section-heading>
  <grounded-compact-list>
  {% for item in section["items"] %}
    <grounded-compact-item>
      <span slot="name">{{ grounded_link(item["type"], item["id"], item["label"], "plain", item.get("fragment")) | safe }}</span>
...
<section class="semantic-section">
  ...
  {% for section in sections %}
    <li>
      ...
      <div>
        <h3>{{ grounded_link(section.kind, section.id, display_name(section), "plain") | safe }}</h3>
...
{% if sources %}
<section class="semantic-section">
  <grounded-section-heading>Document Sources</grounded-section-heading>
  <grounded-compact-list>
  {% for source in sources %}
    <grounded-compact-item>
      <span slot="name">{{ grounded_link(source.kind, source.id, display_name(source), "plain") | safe }}</span>
...
{% if sources %}
<section class="semantic-section">
  <grounded-section-heading>Source Specs</grounded-section-heading>
  <grounded-compact-list>
  {% for source in sources %}
    <grounded-compact-item>
      <span slot="name">{{ grounded_link(source.kind, source.id, display_name(source), "plain") | safe }}</span>
...
{% if assets %}
<section class="semantic-section">
  <grounded-section-heading>Assets</grounded-section-heading>
  <div class="pd-cards semantic-cards">
  {% for asset in assets %}
    <grounded-unit-card>
      <h3 class="pd-card-name">{{ grounded_link(asset.kind, asset.id, display_name(asset), "card-title") | safe }}</h3>
...
{% if consumers %}
<section class="semantic-section">
  <grounded-section-heading>Used By Documents</grounded-section-heading>
  <div class="link-list">
  {% for consumer in consumers %}{{ grounded_link(consumer.kind, consumer.id, display_name(consumer), "plain") | safe }}{% endfor %}
  </div>
</section>
{% endif %}
{% endblock %}
...
{% if docs %}
<section class="semantic-section">
  <grounded-section-heading>Documents In This Set</grounded-section-heading>
  <ol class="timeline-list">
  {% for doc in docs %}
    <li>
      <span class="timeline-number">{{ loop.index }}</span>
...
{% if used_by %}
<section class="semantic-section">
  <grounded-section-heading>Used By</grounded-section-heading>
  <div class="link-list">
  {% for target in used_by %}{{ grounded_link(target.kind, target.id, display_name(target), "plain") | safe }}{% endfor %}
  </div>
</section>
{% endif %}
{% endblock %}
...
{% if tests %}
<section class="semantic-section">
  <grounded-section-heading>Proof Obligations</grounded-section-heading>
  <grounded-compact-list>
  {% for test in tests %}
    <grounded-compact-item>
      <span slot="name">{{ grounded_link(test.kind, test.id, display_name(test), "plain") | safe }}</span>
...
<section class="semantic-section fix-panel">
  <grounded-section-heading>How To Fix Drift</grounded-section-heading>
...
{% if tests %}
<section class="semantic-section">
  <grounded-section-heading>Checked By</grounded-section-heading>
  <div class="link-list">
  {% for test in tests %}{{ grounded_link(test.kind, test.id, display_name(test), "plain") | safe }}{% endfor %}
  </div>
</section>
{% endif %}
{% endblock %}
...
<section class="semantic-lead-card">
  ...
  <div class="artifact-facts">
    ...
    {% if target %}<span><strong>Target</strong>{{ grounded_link(target[0].kind, target[0].id, display_name(target[0]), "plain") | safe }}</span>{% endif %}
  </div>
</section>
{% endblock %}
""",
...
<section class="semantic-section fix-panel">
  <grounded-section-heading>Suggested Improvement</grounded-section-heading>
...
<grounded-page-hero>
  ...
  <span slot="description">Every emitted documentation file should have an owning Grounded spec and a path readers can inspect.</span>
  <p slot="actions" class="background-link"><a href="{{ docs_home_href }}">Overview</a> <a href="{{ document_graph_href }}">Document graph</a></p>
</grounded-page-hero>
<grounded-unit-section>
  <grounded-section-heading divider>Artifact Manifest</grounded-section-heading>
  <grounded-compact-list>
  {% for artifact in document_artifacts(registry) %}
    <grounded-compact-item>
      <span slot="name">{{ grounded_link(artifact.spec.kind, artifact.spec.id, artifact.path, "plain") | safe }}</span>
...
<grounded-unit-section>
  <grounded-section-heading divider>{{ field_value(doc, "output_path") }}</grounded-section-heading>
  <ol class="timeline-list">
  {% for section in specs_for_refs(doc, registry, "section_refs") %}
    <li>
      ...
      <div>
        ...
        {% for source in sources %}{{ grounded_link(source.kind, source.id, display_name(source), "plain") | safe }}{% endfor %}
        </div>
        {% endif %}
      </div>
    </li>
