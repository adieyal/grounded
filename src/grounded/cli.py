...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        prog="grounded", description="Executable project memory."
    )
    parser.add_argument(
        "--root",
        type=Path,
        default=None,
        help="Project root. Defaults to nearest grounded.yml or cwd.",
    )
    ...
    init_parser.add_argument(
        "--grounded-dir",
        type=Path,
        default=Path(".grounded"),
        help="Directory for Grounded project files. Defaults to .grounded.",
    )
    init_parser.add_argument(
        "--update-agents",
        action="store_true",
        help="Add or update the Grounded section in AGENTS.md. Defaults to off.",
    )
    ...
    if args.command == "validate":
        return _print_issues(
            config.root, registry.issues, success="Grounded specs are valid."
        )
    ...
    if args.command == "render":
        ...
        if stale:
            ...
            return 1
        print(
            "Grounded generated views are current."
            if args.check
            else "Rendered Grounded generated views."
        )
        return 0
