...
class GroundedConfig:
    ...
    docs_description: str = (
        "Generated from typed Grounded specs. Search, backlinks, and "
        "links are derived from the same registry."
    )
    docs_nav_label: str = "Project Docs"
    docs_background_title: str = "Grounded background"
    ...
    @classmethod
    def default(
        cls, root: Path, *, grounded_dir: Path | str = ".grounded"
    ) -> "GroundedConfig":
        grounded_path = Path(grounded_dir)
        if not grounded_path.is_absolute():
            grounded_path = root / grounded_path
        return cls(
            root=root,
            specs_dir=grounded_path / "specs",
            type_registry_path=grounded_path / "registry" / "spec-types.json",
            schemas_dir=grounded_path / "schemas",
            templates_dir=grounded_path / "renderers" / "templates",
            styles_dir=grounded_path / "styles",
