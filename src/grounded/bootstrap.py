...
AGENTS_MARKER_START = "<!-- grounded:start -->"
AGENTS_MARKER_END = "<!-- grounded:end -->"
...
Grounded registry units are strongly typed, schema-validated nodes.
...
def init_project(
    ...
    *,
    force: bool = False,
    grounded_dir: Path | str = ".grounded",
    update_agents: bool = False,
) -> list[Path]:
    root = root.resolve()
    config = GroundedConfig.default(root, grounded_dir=grounded_dir)
    created: list[Path] = []

    created.extend(
        _write_if_missing(root / "grounded.yml", _grounded_yml(root, config), force=force)
    )
    created.extend(
        _write_if_missing(
    ...
    created.extend(
        _write_if_missing(
            root / "skills" / "grounded-project-memory" / "SKILL.md",
            _skill(),
            force=force,
        )
    )
...
def _grounded_yml(root: Path, config: GroundedConfig) -> str:
    ...
    return _json(
        {
            ...
            "type": "concept",
            "name": "Grounded project memory",
            "owner": "project",
            "status": "active",
            "description": "Explains how this project uses Grounded specs as durable project memory.",
            "summary": "Durable project knowledge is owned by canonical Grounded specs and referenced by other artifacts.",
            "references": [],
        }
    )
    ...
    return _json(
        {
            "id": "PROJECT-RULE-002",
            "type": "business_rule",
            "name": "Agents maintain Grounded during project work",
            "owner": "project",
            "status": "active",
            "description": "Defines when agents must update Grounded during project work.",
            "statement": "Agents must use, create, and maintain Grounded specs whenever project work changes durable knowledge.",
            "references": [
                "PROJECT-CONCEPT-001",
                "PROJECT-RULE-001",
            ],
    ...
    return _json(
        {
            ...
            "owner": "project",
            "status": "active",
            "description": "Defines the command used to verify that a newly initialized Grounded project is valid.",
            "target": "PROJECT-GAP-001",
            "command": "grounded validate && grounded audit",
...
name: grounded-project-memory
description: Use Grounded as the primary source of truth for specs, durable project knowledge, generated docs, and drift checks.
---
...
15. Update Grounded in the same change when work introduces or changes durable knowledge.
16. Regenerate views with `grounded render`.
17. Run `grounded validate` and `grounded audit`.
18. Report changed Grounded IDs, or state that no durable knowledge changed.

## Non-Negotiables

- Do not duplicate durable facts across artifacts.
- Do not leave Grounded stale after changing durable project meaning.
...
- Do not directly edit bundled Grounded templates, validation code, or rendering functions for project-specific behavior; extend through the type registry, JSON Schemas, project template overrides, styles, slice metadata, or verification specs instead.
...
When reporting work, include changed Grounded IDs and validation results.
"""
