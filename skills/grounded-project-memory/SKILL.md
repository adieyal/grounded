...
6. Check `grounded/registry/spec-types.json` before inventing a spec kind.
...
16. Update Grounded in the same change when work introduces or changes durable knowledge.
17. Regenerate views with `grounded render`.
18. Run `grounded validate` and `grounded audit`.
19. Report changed Grounded IDs, or state that no durable knowledge changed.
...
- Do not manually edit files under `grounded/generated`.
