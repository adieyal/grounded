---
name: grounded-bounded-modeling
description: Use when introducing Grounded into an existing codebase, defining only a subset of the system to model, or helping a user bound, type, and interlink a coherent slice before expanding further.
last_updated: 2026-05-27
---

# Grounded Bounded Modeling

Use this skill when the user wants Grounded to model a subset of a system rather than the whole codebase. The goal is to help the user and the LLM agree on a small, coherent slice, then turn that slice into canonical Grounded units with stable links.

This skill is the canonical workflow for:
- interactive scope definition
- subset-first Grounded bootstrapping in existing codebases
- choosing the right unit kinds and granularity
- connecting new units back to canonical owners without duplication
- rendering canonical owners with `grounded-link`, including inline prose links, labels, and fragment links when a unit, field, tag, or goal is mentioned in a sentence
...
## Canonical Grounded Owners

- [PROJECT-CONCEPT-002](../../grounded/specs/concepts/PROJECT-CONCEPT-002.json): bounded modeling slice
- [PROJECT-WORKFLOW-001](../../grounded/specs/workflows/PROJECT-WORKFLOW-001.json): scope a bounded Grounded model

## Use This Skill When

- the user says they do not want to model the whole system
- Grounded is being introduced into an existing repo
...
When referring to a canonical unit in prose, prefer an inline `grounded-link` over a bare ID. Use the short display label when it improves readability, and keep the canonical ID as the actual target. If the sentence is about a goal, rule, workflow, or owner, link the relevant canonical unit on first mention rather than collecting the links at the end.
